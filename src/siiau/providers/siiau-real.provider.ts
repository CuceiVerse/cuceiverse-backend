...
export class SiiauRealProvider implements SiiauProvider {
  ...
  constructor() {
    this.timeoutMs = Number(process.env.SIIAU_TIMEOUT_MS ?? 25000);
    this.verifyTls = (process.env.SIIAU_VERIFY_TLS ?? "true").toLowerCase() === "true";
    ...
    // axios-cookiejar-support typings no son perfectos con axios v1 (ESM).
    // Solución práctica: create() como any y wrapper() como any.
    ...
    instance.defaults.jar = this.jar;

    this.http = wrapper(instance) as unknown as AxiosInstance;
  }

  async fetchSnapshot(input: SiiauSnapshotRequestDto): Promise<SiiauSnapshotDto> {
    ...
    const { pidm, menuHtml, menuUrl } = await this.openMenuSistemaAndPidm(entry.url, entry.html);

    const alumnos = await this.goToAlumnosUni(menuUrl, menuHtml);

    const registroMenu = await this.openByText(alumnos.url, alumnos.html, "REGISTRO", carreraPrefer);
    const lista = await this.openByText(registroMenu.url, registroMenu.html, "Lista", carreraPrefer);

    const { courses } = parseRegistroLista(lista.html);

    const { majrp, ciclo } = this.resolveCareer(carreraPrefer, cicloPrefer);
    if (!majrp) throw new Error("No pude resolver majrp. Usa carreraPrefer como INNI-202210.");

    const oferta = await this.fetchOferta(pidm, majrp, ciclo ?? "202210", lista.url);
    ...
    const withSchedule = merged.filter((c) => (c.sessions?.length ?? 0) > 0).length;
  ...
  // ---------------- helpers ----------------
  private async sleepJitter() {
    const ms = Math.floor(this.minSleepMs + Math.random() * (this.maxSleepMs - this.minSleepMs));
    await new Promise((r) => setTimeout(r, ms));
  }

  private resolveCareer(carreraPrefer?: string, cicloPrefer?: string): { majrp: string; ciclo?: string } {
  ...
  private async loginMw(codigo: string, nip: string): Promise<string> {
    ...
    const rGet = await this.http.get(this.URL_LOGIN, { headers: { Referer: this.URL_LOGIN } });
    const htmlGet = rGet.data as string;

    const viewState = extractViewState(htmlGet);
    if (!viewState) throw new Error("No se pudo obtener javax.faces.ViewState en login.");
    ...
    await this.sleepJitter();
    const rPost = await this.http.post(this.URL_LOGIN, new URLSearchParams(payload), {
      headers: {
  ...
  private async jumpToEscolar(dashboardHtml: string): Promise<{ url: string; html: string }> {
    ...
    const enlace = $("a")
      .toArray()
      .find((a) => (($(a).attr("onclick") ?? "").includes("mojarra.jsfcljs") && ($(a).attr("onclick") ?? "").includes("sistemasForm")));
    if (!enlace) throw new Error("No se encontró botón a Escolar (mojarra.jsfcljs).");
    ...
    await this.sleepJitter();
    const rPuente = await this.http.post(urlDestino, new URLSearchParams(payload), {
      maxRedirects: 0,
      headers: {
        Referer: this.URL_LOGIN,
        Origin: "https://mw.siiau.udg.mx",
    ...
    await this.sleepJitter();
    const rEntry = await this.http.get(nextUrl, { headers: { Referer: this.URL_LOGIN } });
    return { url: nextUrl, html: rEntry.data as string };
  }

  private async openMenuSistemaAndPidm(entryUrl: string, entryHtml: string): Promise<{ pidm: string; menuHtml: string; menuUrl: string }> {
    const frames = parseFrames(entryHtml).map((f) => ({ name: f.name, src: urlJoin(entryUrl, f.src) }));
    ...
    await this.sleepJitter();
    const rMain = await this.http.get(mainUrl, { headers: { Referer: entryUrl } });
    const mainHtml = rMain.data as string;

    const fInicio = findFormByName(mainHtml, "fInicio");
    if (!fInicio || fInicio.method !== "POST") throw new Error("No se encontró fInicio POST.");

    const fInicioUrl = urlJoin(mainUrl, fInicio.action);

    await this.sleepJitter();
    const rVal = await this.http.post(fInicioUrl, new URLSearchParams(fInicio.inputs), {
      headers: { Referer: mainUrl, Origin: this.BASE_ESCOLAR, "Content-Type": "application/x-www-form-urlencoded" },
    });
    ...
    const baseForMainPage = ((rVal as any).request?.res?.responseUrl ?? fInicioUrl) as string;
    const mainPageUrl = urlJoin(baseForMainPage, mainPage.action);

    await this.sleepJitter();
    const rFm = await this.http.post(mainPageUrl, new URLSearchParams(mainPage.inputs), {
      headers: { Referer: fInicioUrl, Origin: this.BASE_ESCOLAR, "Content-Type": "application/x-www-form-urlencoded" },
    });

    const fmHtml = rFm.data as string;
    const frames2 = parseFrames(fmHtml).map((f) => ({ name: f.name, src: urlJoin(mainPageUrl, f.src) }));
    const menuUrl = frames2.find((f) => f.name === "Menu" || f.src.toLowerCase().includes("gupmenug.menu_sistema"))?.src;
    if (!menuUrl) throw new Error("No se encontró frame Menu (menu_sistema).");

    await this.sleepJitter();
    const rMenu = await this.http.get(menuUrl, { headers: { Referer: mainPageUrl } });
    return { pidm, menuHtml: rMenu.data as string, menuUrl };
  }

  private async goToAlumnosUni(menuUrl: string, menuHtml: string): Promise<{ url: string; html: string }> {
    const $ = loadHtml(menuHtml);
    const link = $("a")
      .toArray()
      .map((a) => $(a).attr("href") ?? "")
      .find((href) => /(?:\?|&)p_sistema_c=ALUMNOS(?:&|$)/.test(href) && !href.toUpperCase().includes("SEMS"));
    if (!link) throw new Error("No encontré módulo ALUMNOS (UNI) en menu_sistema.");
    ...
    const finalUrl = ((r as any).request?.res?.responseUrl ?? target) as string;
  ...
  private setSelectedCarreraFromPage(html: string, prefer?: string) {
    ...
    if (!sel.length) return { carreraValue: null as string | null, majrp: null as string | null, ciclo: null as string | null };

    const options = sel.find("option").toArray().map((o) => ({
  ...
  private async openByText(baseUrl: string, html: string, label: string, carreraPrefer?: string): Promise<{ url: string; html: string }> {
    ...
    $("a").each((_, a) => {
      ...
      if (t === label.toUpperCase()) aEl = a;
    });
    if (!aEl) {
      $("a").each((_, a) => {
        const t = textOf($(a)).toUpperCase();
        if (t.includes(label.toUpperCase())) aEl = a;
      });
    }
    ...
    else if (href.toLowerCase().startsWith("javascript:")) target = urlFromJs(href);
    ...
    const finalUrl = ((r as any).request?.res?.responseUrl ?? abs) as string;
    return { url: finalUrl, html: r.data as string };
  }

  private async fetchOferta(pidm: string, majrp: string, cicloDesired: string, referer: string): Promise<{ url: string; html: string }> {
    // GET form
    const formUrl = `${this.WAL}/sgpofer.secciones?pidmp=${pidm}&majrp=${majrp}`;
    await this.sleepJitter();
    const rForm = await this.http.get(formUrl, { headers: { Referer: referer } });
    const formHtml = rForm.data as string;

    const $ = loadHtml(formHtml);
    const formTag = $("form[name='frm_consulta_oferta']").first().length ? $("form[name='frm_consulta_oferta']").first() : $("form").first();

    const action = (formTag.attr("action") ?? "sspseca.consulta_oferta").trim();
    const base = ((rForm as any).request?.res?.responseUrl ?? formUrl) as string;
    ...
    const resHtml = rRes.data as string;
    const finalUrl = ((rRes as any).request?.res?.responseUrl ?? postUrl) as string;
    return { url: finalUrl, html: resHtml };
  }
}